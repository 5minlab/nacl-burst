...
namespace NaCl
{
    ...
    internal static unsafe class NaClBurstBackend
    {
        ...
        internal static void Preflight()
        {
            ...
            _ = CryptoSecretBoxNonceWithNoncePrefix(message, key, noncePrefix);
            _ = CryptoSecretBoxOpenNonce(ciphertext, key);
            _ = CryptoSecretBox(message, nonce, key);
            _ = CryptoSecretBoxOpen(payload, nonce, key);
            _ = CryptoBoxKeypair(secretKey);
            _ = CryptoBoxBeforenm(publicKey, secretKey);
        }

        internal static byte[] CryptoSecretBoxNonce(byte[] message, byte[] secretKey)
        {
            ...
            return CryptoSecretBoxNonceWithNoncePrefix(message, secretKey, noncePrefix);
        }

        internal static byte[] CryptoSecretBoxNonceWithNoncePrefix(byte[] message, byte[] secretKey, ReadOnlySpan<byte> noncePrefix)
        {
            if (!NaClInputValidation.IsValidSecretBoxEncryptInput(message, secretKey) ||
        ...
        internal static byte[] CryptoSecretBoxOpenNonce(byte[] ciphertext, byte[] secretKey)
        {
        ...
        internal static byte[] CryptoSecretBox(byte[] message, byte[] nonce, byte[] secretKey)
        {
        ...
        internal static byte[] CryptoSecretBoxOpen(byte[] payload, byte[] nonce, byte[] secretKey)
        {
        ...
        internal static byte[] CryptoBoxKeypair(byte[] secretKey)
        {
        ...
        internal static byte[] CryptoBoxBeforenm(byte[] publicKey, byte[] secretKey)
        {
